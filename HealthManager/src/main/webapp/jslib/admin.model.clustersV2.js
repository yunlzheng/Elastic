			...
			if(data[i] != 0 && data[i] != 1){
				...
				if(type == 1){
					tbody = virtualOption.tr.replace(/{serialNumber}/g,(index))
					 						...
					 						.replace(/{Operation}/,"<a href='sdetail.html?ip=" + ip +"'>点击查看</a>");
				}else if(type == 0){
					tbody = virtualOption.tr.replace(/{serialNumber}/g,(index))
											.replace(/{virtualIp}/g,data[i])
											.replace(/{virtualStatus}/g,"<span class='label label-success' >运行中</span>")
...
function pageFooter(footerTarget){
	$(footerTarget).empty()
				   ...
				   .append('<div class="table-pagination"><span class="pagination-info">' +
						   '第<input type="text" value="0">页 / 共 0页 </span><a class="pagination-button disable" rel="first">' +
...
function privateSuccessCallBack(data){
	...
	if(data!=null&&data!=undefined&&data.length>0){
		...
		virtalList($.proxy(virtalListCallBack,{"treeName":this.treeName,"requestType":"private"}),virtalListError,id);
...
function privateErrorCallBack(){
	//console.log("privateErrorCallBack");
}
...
function RequestTypeById(treeNode){
