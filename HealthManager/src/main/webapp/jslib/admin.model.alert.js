...
function chartData(success,error){
  ...
  function charthandle(mailnum,smsnum){
	  ...
	  for(var i = 0 ; i < data.length ; i++){
		  if(data[i].type == 1){
			 ...
			 $("<tr><td>"+(index)+"</td><td>" + data[i].joinTime + "</td>" +
				   		...
				   		"<td>" + data[i].message + "</td><td><a excId='" + data[i]["id"]  + "' class='alertdetail'>详情</a></td></tr>").appendTo("#body_content");
  ...
  function appendMessage(index,data){
	  ...
	  $("<tr><td>"+(index)+"</td>" +
			    ...
			    "<td>"+tracsform(data.type)+"</td>" +
	  			"<td>" + data.joinTime + "</td>" +
		   		"<td>"+data.key+"</td>"+
		   		"<td>" + data.message + "</td><td><a excId='" + data["id"]  + "'  class='alertdetail'>详情</a></td></tr>").appendTo("#body_content");
