$(document).ready(function() {
	...
	$(window).scroll(function(){
		...
		function chartSuccess(data){
			...
			var mail_arr = new Array();
			var message_arr = new Array();
			var name_arr = new Array();
			
			for(var i = 0 ; i < data.length ; i++){
				mail_arr.push(data[i].mailNum);
				message_arr.push(data[i].messageNum);
				name_arr.push(data[i].name);
			}
			
			charthandle(name_arr,mail_arr,message_arr);
	   ...
	   function getAllSuccess(data){
		...
		var EXCVIEW = {
				...
				detail:'<div class="item"><label>发生时间:</label><span>{startTime}</span></div>'+
						'<div class="item"><label>事件源:</label><span>{address}</span></div>'+
