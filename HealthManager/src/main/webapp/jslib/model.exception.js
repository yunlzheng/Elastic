...
var sendAjaxRequest = function(success, error, id) {
	/**
	 * 如果size为空则获取所有的记录
	 */
...
function addDataToBody(data,index){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		var tr_1 = "<tr><td>"+(index)+"</td><td>" + data[i]["name"] + "</td><td>"
				+ data[i]["startTime"] + "</td><td>" + data[i]["endTime"]
				+ "</td><td>" + keepTime + "*24h</td><td>" + data[i]["message"];
		var tr_3 = "<td></tr>";
		var tr_2 = "";
		var tr = "";
 ...
 * key:value;key:value
 * */
function searchException(eqs,likes){
