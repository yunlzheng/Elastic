...
var  scaleRequest = function(success,error){
	...
	$.ajax({
		...
		type : "get",
		url : "../rest/combin/user/exception/cate/scale",
		success : success,
		error : error,
		timeout : 3000
...
var sendAjaxRequest = function(success, error, type, level,name) {
	/**
	 * 如果size为空则获取所有的记录
	 */
...
function isMore(error){
	...
	$("#more").empty().html("<img src='../assets/images/ajax-loader.gif'/><span>数据加载中...</span>");
...
function addDataToBody(data,index){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		var tr = "<tr><td>"+(index)+"</td><td>" + data[i]["name"] + "</td><td>"
				+ data[i]["startTime"] + "</td><td title='" + data[i]["message"]+"'>" + data[i]["message"]+"</td><td>"+transformExceptionLevel(data[i]["level"])+"</td>" +
